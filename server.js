...
// ===== Settings (set these in Railway Variables) =====
...
const ADMIN_PASS  = process.env.ADMIN_PASS  || "admin12345"; // CHANGE in Railway
// ====================================================
...
// ===== Client API =====
// Frontend uses: /api/check?code=XXXX
...
app.put("/api/admin/cases/:id", authMiddleware, (req, res) => {
  ...
  const expires_at = (ttl_days !== undefined)
    ? (Number(ttl_days || 0) > 0 ? addDaysIso(Number(ttl_days || 0)) : null)
    : undefined;

...
const JWT_SECRET = process.env.JWT_SECRET || "CHANGE_ME_SUPER_LONG_SECRET_123456789_CHANGE_ME";
const ADMIN_USER = process.env.ADMIN_USER || "admin";
const ADMIN_PASS = process.env.ADMIN_PASS || "admin12345"; // CHANGE in Railway variables!
const VOLUME_PATH = process.env.VOLUME_PATH || ""; // set to /data when using Railway Volume mounted at /data
const DATA_DIR = VOLUME_PATH ? VOLUME_PATH : __dirname;
// ====================
...
// Healthcheck (Railway likes this)
...
function initDb() {
  db.serialize(() => {
    ...
    db.run(`CREATE INDEX IF NOT EXISTS idx_cases_expires ON cases(expires_at)`);
  });
}
initDb();
...
app.post("/api/admin/cases", authMiddleware, (req, res) => {
  ...
  if (!Number.isNaN(ttl) && ttl > 0) {
    const allowedTtl = [3, 5, 7, 14];
    if (!allowedTtl.includes(ttl)) return res.status(400).json({ ok: false, message: "Bad ttl." });
    expires_at = addDaysIso(ttl);
  }

  db.run(
    `INSERT INTO cases (code, status, title, note, is_active, created_at, expires_at)
     VALUES (?, ?, ?, ?, 1, ?, ?)`,
    [code, st, title || "Invitation status", note || "", nowIso(), expires_at],
    function (err) {
...
// Delete whole case (and all its files)
...
function cleanupExpired() {
  ...
  db.all(
    `SELECT id FROM cases WHERE expires_at IS NOT NULL AND expires_at <= ?`,
    [now],
    (err, rows) => {
      if (err) return;
      for (const r of rows || []) {
        db.all(`SELECT stored_name FROM files WHERE case_id = ?`, [r.id], (e2, frows) => {
          const stored = (frows || []).map(x => x.stored_name);
          db.run(`DELETE FROM cases WHERE id = ?`, [r.id], () => {
            for (const name of stored) fs.unlink(path.join(UPLOAD_DIR, name), () => {});
          });
        });
      }
    }
